...
def create_database():
    mydb = mysql.connector.connect(host="localhost", user="root", passwd="NewProject@123", database="stock")
...
def sale_product():
    ...
    for x in mycursor:
        ...
        if cnt != 0:
            ...
            if qty <= pqty:
                sql = "select max(salesid)from sales"
                mycursor.execute(sql)
                for x in mycursor:
                    sid = x[0]
                ...
                val = (sid + 1, datetime.datetime.now(), pcode, price, qty, total)
                mycursor.execute(sql, val)
                sql = "UPDATE product SET pqty=pqty-%s WHERE pcode=%s"
                val = (qty, pcode)
                mycursor.execute(sql, val)
